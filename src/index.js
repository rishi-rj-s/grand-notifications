...
/** @typedef {'success' | 'error' | 'info' | 'warning' | 'custom'} ToastType */
/** @typedef {'top-left' | 'top-right' | 'bottom-left' | 'bottom-right' | 'top-center' | 'bottom-center' | 'middle'} ToastPosition */
/** @typedef {'slide' | 'bounce' | 'fade'} ToastEntranceAnim */
/** @typedef {'melt' | 'pixel' | 'hologram'} ToastExitAnim */

/**
 * @typedef {Object} ToastOptions
 * @property {ToastPosition=} position
 * @property {ToastType=} type
 * @property {ToastEntranceAnim=} entranceAnim
 * @property {ToastExitAnim=} exitAnim
...
function showToast(options = {}) {
     try {
          ...
          toast.dataset.exitAnim = exitAnim;
          ...
          if (closable) {
               ...
               closeBtn.addEventListener('click', () => {
                    const storedExitAnim = toast.dataset.exitAnim && validExitAnims.includes(toast.dataset.exitAnim) ? toast.dataset.exitAnim : exitAnim;
                    dismissToast(toast, /** @type {ToastExitAnim} */(storedExitAnim), 200);
               });
          ...
          const updateProgress = (timestamp) => {
               ...
               if (elapsed >= duration) {
                    const storedExitAnim = toast.dataset.exitAnim && validExitAnims.includes(toast.dataset.exitAnim) ? toast.dataset.exitAnim : exitAnim;
                    dismissToast(toast, /** @type {ToastExitAnim} */(storedExitAnim), speed);
                    return;
               }
 ...
 * @param {ToastExitAnim} [exitAnim] - Animation used when toast disappears
...
function dismissToast(toast, exitAnim = 'melt', speed = 600) {
     ...
     const progress = /** @type {HTMLProgressElement | null} */ (toast.querySelector('.grand-toast-progress'));
     if (progress) {
          progress.style.transform = progress.style.transform;
     }
     ...
     requestAnimationFrame(() => {
          ...
          const validExitAnims = ['melt', 'pixel', 'hologram'];
          const selectedExitAnim = toast.dataset.exitAnim && validExitAnims.includes(toast.dataset.exitAnim) ? toast.dataset.exitAnim : exitAnim;

          switch (selectedExitAnim) {
               case 'pixel':
